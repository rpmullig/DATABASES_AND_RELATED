...
SELECT CUSTOMERS.ID, FIRST_NAME, LAST_NAME, ADDRESS1, ADDRESS2, CITY, STATE, ZIP, PHONE, ORDERS.CUSTOMER_ID, SUM(PRICE*QTY) AS ORDERTOTAL
FROM CUSTOMERS, ORDERS, ORDER_DETAILS
WHERE CUSTOMERS.ID=ORDERS.CUSTOMER_ID
AND ORDER_DETAILS.ORDER_ID=ORDERS.ORDER_NUMBER
GROUP BY CUSTOMERS.ID, FIRST_NAME, LAST_NAME, ADDRESS1, ADDRESS2, CITY, STATE, ZIP, PHONE, ORDERS.CUSTOMER_ID
HAVING SUM(PRICE*QTY)>(SELECT AVG(SUM(PRICE*QTY))
FROM ORDER_DETAILS
GROUP BY ORDER_ID);
...
/* Part B */
UPDATE CUSTOMERS
SET CUSTOMERS.LAST_NAME='Subramanian'
WHERE CUSTOMERS.LAST_NAME='Subramani';
...
/* Part D */
Select SUM (QTY) AS TOTAL, PRODUCT_ID
FROM ORDER_DETAILS
GROUP BY PRODUCT_ID
HAVING SUM(QTY)=
(SELECT MIN(TOTAL) FROM
(SELECT SUM(QTY) AS TOTAL,PRODUCT_ID FROM ORDER_DETAILS
GROUP BY PRODUCT_ID));

/* Part E */    
select CUSTOMERS.FIRST_NAME, CUSTOMERS.LAST_NAME,  SUM(ORDER_DETAILS.PRICE * ORDER_DETAILS.QTY) as CR
FROM CUSTOMERS
JOIN ORDERS ON CUSTOMERS.ID = ORDERS.CUSTOMER_ID
JOIN ORDER_DETAILS ON ORDERS.ORDER_NUMBER = ORDER_DETAILS.ORDER_ID
